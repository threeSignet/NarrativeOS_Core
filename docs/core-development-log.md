...
## 2026-06-25 · Phase 8 全面审核修复 + P0-P2 清零

### 会话总览

对 Phase 7+8 累积代码做全面深度审查（P0-P2 三级），逐项核实后修复。最终 P0/P1 清零，P2 修 15 项，补全 graph-service 测试。

### P0 修复（数据完整性 + 测试稳定性）

1. **softDeleteProject 漏 Phase 8 三张表**（writing-store.ts）：childTables 数组补 writing_relations/writing_associations/writing_relation_hints，防止项目删除留孤儿行。新增回归测试。
2. **updateRelationCandidate 乐观锁缺失**（writing-store.ts）：UPDATE 后检查 changes===0，区分 NOT_FOUND / VERSION_CONFLICT，对齐 updateProposalView 范式。
3. **audit-list.test.ts setTimeout flaky**：1100ms sleep 改为显式时间戳 INSERT，测试耗时 2200ms→36ms。

### P1 修复（并发安全 + 审计完整性 + 类型安全）

4. **CoreBridge 4 通道审计补全**（real-bridge.ts）：commitReviewedKnowledgeChange/Retcon/WorldPackage/Thread 补 sourceRefs + writebackError partial 审计 + WritingCoreRef 双向索引。
5. **ensureNode attributes 传递**（graph-service.ts）：6 处调用补 entityAttributes 参数，间接创建节点不再丢失属性。
6. **WRITING_TABLE_NAMES 补三表**（filter.ts）：修复 normal 模式下 Phase 8 三张表名裸值不掩码。
7. **6 处无效断言修复**（narrative-agent/push-mode/end-to-end/tool-router/w13）：toBeGreaterThanOrEqual(0) 替换为有意义的验证。

### P2 修复（代码质量 + 类型安全，15 项）

**Core 层**（11 项）：
- tool-router：新增 predicates/rules 输入校验、消除双重类型断言、catch 保留原始错误、提取 makeToolContext 方法、PredicateExtension/RuleExtension 显式类型构造消除 as any
- retcon-engine：删除死变量 targetChapter、提取 collectAffectedFactIds 消除 3 处重复遍历
- proposal-manager：修复模板字符串嵌套 bug（内层单引号内模板变量不插值）、as any → unknown as Record
- fact-renderer：formatFactValue 复用 isEntityRef 消除重复逻辑
- sync-queue-consumer：定义 FactRow 接口消除 3 处 any
- schema-extension-manager：4 条压缩 SQL 格式化
- thread-resolver：删除 void eventSubject 死代码

**写作层**（1 项）：
- types.ts：删除无人引用的 RELATION_CANDIDATE_TRANSITIONS 导出

**测试层**（3 项）：
- tool-router.test.ts：删除死 beforeEach + 无效断言修复
- push-mode-validation.test.ts / end-to-end.test.ts：弱断言替换/移除

### 测试补全

- phase8-relations.test.ts 新增 7 个 GraphView 投影测试（loadEntityAttributes 正常/废弃/过滤、多条件过滤、archived 关联、relationship 模式、自环防御）

### 验证

- `npx tsc --noEmit` exit 0
- 全量回归 **59 文件 815 测试全绿，0 skipped，0 failed**

### 变更文件

- P0+P1+P2 修复（23 文件，+1093/-126）：commit `1b1b71a`
- 测试补全（1 文件，+127）：commit `bea1a1d`

---

## 2026-06-25 · Phase 9：地理与多层宇宙（完整交付）

### 会话总览

Phase 9 从规划到交付：类型定义 → DDL → Store CRUD → 状态机 → SpatialService → SpatialViewService → GraphService 空间投影 → Blueprint 空间类型 → Agent 工具 → CLI 命令 → 测试 → Code Review 修复。

### 交付内容

**类型定义**（types.ts）：
- WritingSpatialNode / WritingSpatialEdge / SpatialView 接口
- SpatialNodeMaturity / SpatialNodeStatus / SpatialEdgeStatus / SpatialEdgeLayer / SpatialEdgeDirection 枚举
- SpatialTraversalRule 通行规则

**持久化层**（writing-store.ts）：
- W.17 writing_spatial_nodes / W.18 writing_spatial_edges / W.19 writing_spatial_views（3 张新表 + 索引）
- 完整 CRUD + 乐观锁 + 行映射
- softDeleteProject childTables 补全 3 张新表
- updateBlueprint 支持 spatialNodeTypes / spatialEdgeTypes 字段

**状态机**（state-machine.ts）：
- validateSpatialNodeMaturity（hint→candidate→confirmed→registered）
- validateSpatialEdgeStatus（candidate→confirmed→submitted→committed/archived）

**SpatialService**（新建）：
- 空间节点：create / update / advanceMaturity / registerToCore / deprecate
- 空间边：create / update / confirm / submit / archive
- 空间视图：create / updateLayout
- registerSpatialNodeToCore 接入 CoreBridge（EntitySketch → approve → registerReviewedEntity → coreEntityId）

**SpatialViewService**（新建）：
- buildSpatialTreeView：可配置 parentEdgeTypeIds，默认 contains/parent_of
- exportSpatialData：完整数据 + 树状视图 JSON

**GraphService 扩展**：
- 空间节点/边投影（confirmed 状态）+ spatial 模式过滤
- ensureSpatialNode + buildSpatialEdgeLabelMap

**BlueprintService 扩展**：
- addSpatialNodeType / addSpatialEdgeType（提取 addSpatialType 共享逻辑）

**Agent 工具**：
- Tool 14: detect_spatial_nodes
- Tool 15: get_spatial_view

**CLI 命令**：
- /spatial add-node / add-edge / confirm-edge / list
- /map（GraphService spatial 模式）
- /blueprint add-spatial-type node|edge

**Code Review 修复**：
- registerSpatialNodeToCore 接入 CoreBridge（原为 TODO 桩）
- buildSpatialTreeView PARENT_EDGE_TYPES 参数化
- addSpatialNodeType/EdgeType 重复逻辑提取共享方法

### 验证

- `npx tsc --noEmit` exit 0
- 352 writing 测试全绿（含 16 新用例）
- commit `5be00c7`

### Phase 9 完整度

Roadmap 6 项全部完成。Feature-Spec §9.5-§9.9（角色位置/可达性/多层宇宙/联动/导入）推迟到后续 Phase。

### 下一步：Phase 10（章节、场景与时间线）



## 2026-06-29 · Phase 10：章节、场景与时间线（Feature-Spec §14 + §15）

### 会话总览

Phase 10 建立「作品结构 + 时间线」纵向能力：章节规划 → 场景规划 → 时间线视图。让作者能按章节组织叙事结构，并在统一时间线上看到「已提交 Core 事件」与「章节/场景计划」的合并视图。

### 交付内容

**Part A：新功能（Feature-Spec §14 章节/场景/结构规划 + §15 时间线系统）**

- **类型定义**（types.ts）：ChapterPlan / ScenePlan 接口 + ChapterPlanStatus / ScenePlanStatus / SceneConflict / ScenePurpose 等枚举。
- **状态机**（state-machine.ts）：
  - validateChapterPlanMaturity（outline→drafted→writing→done→revising→finalized）
  - validateScenePlanStatus（idea→outlined→drafted→simulated→committed_to_world→prose_written→done→revising）
- **持久化层**（writing-store.ts）：
  - W.20 writing_chapter_plans / W.21 writing_scene_plans（2 张新表 + 索引）
  - 完整 CRUD + 行映射；softDeleteProject childTables 覆盖 21 张表
- **ChapterService**（新建）：创建章节（带 order）/ 更新 / 状态推进 / 重排（reorder）。
- **SceneService**（新建）：创建场景（关联 chapterId）/ 更新 / 状态推进 / 重排；支持按章节过滤。
- **TimelineService**（新建）：buildTimelineView——合并 Core 已提交事件（eventStore）+ 章节计划 + 场景计划，按 sourceLayer 区分（committed/planned），支持 world / chapter 两种模式。
- **Agent 工具**（tool-router.ts）：
  - Tool 16: create_chapter_plan
  - Tool 17: create_scene_plan
  - Tool 18: get_timeline_view
  - setChapterSceneServices 注入三服务
- **CLI 命令**（command-handlers.ts + chat.ts）：
  - /chapter list | /chapter add <标题> [--order N]
  - /scene list [--chapter <id>] | /scene add <chapterId> <标题> [--order N]
  - /timeline（时间线视图）

**Part B：查漏补缺**

- IdeaService discard/restore：经核查 store 层已有 validateIdeaTransition 校验，非真缺口（Gap Register 基线「~85% discard/restore 绕状态机」结论修正）。
- labels.ts 单元测试：8 用例覆盖 4 个标签函数 + 未知枚举降级（tests/unit/labels.test.ts）。

### 验证

- `npx tsc --noEmit` exit 0
- 409 测试全绿（含 chapter-service 12 + timeline-service 6 新用例）
- 工具计数 13→15→18
- commit `0ff5fd4`

### Phase 10 完整度

Roadmap Phase 10 范围（ChapterPlan/ScenePlan/TimelineView/角色行程/场景调度/时间线冲突检查）数据层 + 服务层 + 工具层 + CLI 全部完成。Feature-Spec §14.5 计划事件与世界变化候选、§15.5 角色个人时间线、§15.7 时间线冲突与因果检查推迟到后续 Phase。

### 下一步：Phase 11（读者模型、伏笔与信息释放）



## 2026-07-02 · Phase 11：读者模型、伏笔与信息释放（Feature-Spec §16 + §17）

### 会话总览

Phase 11 建立「读者认知 + 伏笔生命周期 + 信息释放节奏」三层能力。让作者能管理「读者在什么时候知道什么」「伏笔从埋设到回收的完整生命周期」「信息按计划分阶段释放」。分两次提交完成（伏笔/读者主体 + RevealPlan 揭示计划补全）。

### 交付内容

**数据层（types + DDL + Store CRUD）**

- **类型定义**（types.ts）：
  - ReaderAudienceProfile / ReaderKnowledgeState + ReaderAudienceKind / ReaderKnowledgeStateValue 枚举
  - ForeshadowingPlan / HintOccurrence / PayoffPlan + ForeshadowingKind / ForeshadowingPlanStatus / HintIntensity / HintVisibility / PayoffKind 枚举
  - RevealPlan / RevealMilestone + RevealPlanStatus / RevealMilestoneKind 枚举
- **持久化层**（writing-store.ts）：
  - W.22 writing_reader_audiences / W.23 writing_reader_knowledge_states
  - W.24 writing_foreshadowing_plans / W.25 writing_hint_occurrences / W.26 writing_payoff_plans
  - W.27 writing_reveal_plans / W.28 writing_reveal_milestones
  - 共 7 张新表 + 完整 CRUD + 行映射；softDeleteProject childTables 覆盖 28 张表

**服务层**

- **ReaderService**（新建）：createAudience / getOrCreateDefaultAudience（默认目标读者自动创建）/ createKnowledgeState / updateKnowledgeState。核心不变式：ReaderModel 不写 Core Knowledge。
- **ForeshadowingService**（新建）：createForeshadowingPlan / updateForeshadowingPlanStatus / createHintOccurrence / createPayoffPlan / createRevealPlan / updateRevealPlanStatus / createRevealMilestone。核心不变式：伏笔计划不自动写 Core Thread。

**Agent 工具**（tool-router.ts，setReaderForeshadowingServices 注入）

- Tool 19: create_foreshadowing_plan
- Tool 20: get_foreshadowing_plans
- Tool 21: create_reader_knowledge_state
- Tool 22: create_reveal_plan

### 验证

- `npx tsc --noEmit` exit 0
- foreshadowing-service 11 用例 + reader-service 4 用例
- 工具总数 18→22
- commit `d424089`（伏笔/读者主体）+ `e6f8062`（RevealPlan 揭示计划补全）

### Phase 11 完整度

Roadmap Phase 11 范围（ReaderKnowledgeState/RevealPlan/ForeshadowingPlanner/读者可见状态/悬念与误导管理）数据层 + 服务层 + 工具层完成。Feature-Spec §16.5 POV 视角检查、§16.6 角色 Knowledge 对照、§17.5 Core Thread 映射、§17.6 悬念释放检查、§17.7 悬念曲线、§17.10 Retcon 对伏笔的影响推迟到后续 Phase。

### 已知遗留（Phase 12 A1/A2 修复）

- **Tool 20 运行时 bug**：handleGetForeshadowingPlans 通过 `(this as any).store` 访问 ToolRouter 不存在的 store 属性，恒返回空数组。Phase 12 A1 已修复（改走 ForeshadowingService.listForeshadowingPlans）。
- **Phase 11 无 CLI 命令**：reader/foreshadowing/reveal 仅 Agent 可调。Phase 12 A2 已补齐 /reader /foreshadow /reveal 三命令。

### 下一步：Phase 12（正文、修订与产品化前端）



## 2026-07-02 · Phase 12：正文 / 风格 / 修订 / Retcon视图 / 导入导出（数据层闭环）

### 会话总览

Phase 12 完成写作层五大能力的**数据层闭环**（类型 + DDL + Store + Service + 状态机 + CLI + Agent 工具），让后端干净收尾给前端让路。**不含 LLM 生成 / embedding 漂移检测等重 AI 部分**（正文生成、风格检查的 AI 能力留给前端会话或后续 phase）。同步修复前序遗留（Tool 20 运行时 bug + Phase 11 CLI 缺失 + 文档断层）。

期间遭遇并行会话（设定集文档树 DocumentService）并发修改同一批文件，两次 Edit 因 "file modified since read" 失败；按用户决策暂停后恢复，整合无冲突。

### 交付内容

**阶段 A：前序遗留清零**

- **A1 Tool 20 bug 修复**：`get_foreshadowing_plans` 此前通过 `(this as any).store` 访问 ToolRouter 不存在的 store 属性，恒返回空数组。给 ForeshadowingService 补 `listForeshadowingPlans` / `listRevealPlans`，handler 改走 service（3 list 用例 + 2 Tool 20 运行时回归用例）。
- **A2 Phase 11 CLI 命令**：补 `/reader` `/foreshadow` `/reveal` 三命令（reader/foreshadowing/reveal 此前仅 Agent 可调）。顺带补 Phase 10 chapterService/sceneService/timelineService 的 cliDeps 注入（此前 handleChapter 的 add 路径会因服务未注入而失败）。
- **A3 文档补写**：补 Phase 10、Phase 11 开发日志；更新 Gap Register 基线表（纳入 5 个新模块、表数 28、工具数 22）。

**阶段 B：Phase 12 五大能力（数据层闭环）**

- **B1 正文草稿（§13.8）**：
  - W.29 writing_prose_documents + W.30 writing_prose_blocks（2 张新表）
  - ProseDocument / ProseBlock / ProseBlockKind / TextAnchor 类型
  - ProseService：createDocument / addBlock / updateBlock / moveBlock / deleteBlock / getDocumentWithBlocks / ingestText + splitMarkdownToBlocks 纯函数（# 标题/段落/分隔符切分）
  - 核心特性：结构变更 bump versionId（正文锚点失效判断依据）；order_index 连续性维护；跨项目访问拒绝
- **B2 风格系统（§18）**：
  - W.31 writing_style_guides（1:1 容器）+ W.32 writing_style_examples + W.33 writing_banned_expressions（3 张新表）
  - StyleGuide / StyleExample / BannedExpression + NarrativePerson/Distance/PacingPreference 类型
  - StyleService：getOrCreateDefaultGuide / updateGuide / addExample / addBannedExpression + list 方法
  - 不做风格检查/漂移检测（需 embedding）
- **B3 修订系统（§19.1）**：
  - DraftStatus 增 `revising`（committed → revising → committed/drafting），DDL CHECK + 状态机 + labels 同步
  - W.34 writing_revision_records（1 张新表）
  - RevisionRecord / RevisionTargetType / RevisionAction 类型
  - RevisionService：recordRevision / listRevisionsByTarget / listRevisionsByGroup / restoreRevision
  - versionGroupId 自动生成（同对象多次修订归组）；不触发 Core
- **B4 Retcon 可视化（§10.5/§19.4）**：
  - W.35 writing_retcon_reports（1 张新表）
  - RetconImpactReport / RetconAffectedNode / RetconAffectedEdge / WritingArtifactRecheckItem 类型
  - RetconViewService：buildRetconImpactReport（从 Core propose_retcon 投影受影响 Fact/Event/Thread/Knowledge + 写作层重检项）/ getReport / listReports / updateReportStatus
  - 确认前只读，不调 Core commit（§10.5 验收）；重检项含关联伏笔计划
- **B5 导入导出（§20/§23）**：
  - W.36 writing_import_batches（1 张新表）
  - ImportBatch / ImportType 类型
  - ImportExportService：importText（建批次 + 切分建文档，失败保留原文快照）/ exportProject（聚合全量 JSON，复用各 store list）
  - 纯规则切分，不调 LLM 反推 Blueprint（§20.7 推迟）

**阶段 B-CLI：6 个命令**

- `/prose`（list/new/add/查看）、`/style`、`/revision <类型> <id>`、`/retcon`（list/详情）、`/import <文件>`、`/export [scope] [文件]`
- 5 个 Service 实例化 + cliDeps 注入 + help 清单

**阶段 C：聚合收尾**

- **C1 Agent 工具接入**：4 个只读+结构化工具（get_prose_document / get_style_guide / get_retcon_report / export_project），工具数 22→26，setPhase12Services 注入。
- **C2 labels/WRITING_TABLE_NAMES**：revising 标签 + 8 张新表加入 visibility 过滤白名单（Phase 9-11 遗漏的表一并补全）。
- **C3 测试**：5 个 Service 测试文件，42 用例全绿（prose 12 + style 7 + revision 6 + retcon-view 7 + import-export 10）。

### 验证

- `npx tsc --noEmit` exit 0
- `npx vitest run` 全量 930 测试全绿（基线 868 + 62 新增：5 Service 测试 42 + Tool 20 回归 5 + 其他计数更新）
- 工具数 22→26；表数 28→36

### Phase 12 完整度

Roadmap Phase 12 范围（正文草稿/风格系统/修订系统/Retcon可视化/导入导出）数据层 + 服务层 + CLI + Agent 工具全部完成。Feature-Spec §13.3 行内预测/Ghost Text、§13.4 光标跟随反馈、§18.6 风格检查与漂移提示、§19.2 正文版本与锚点恢复的自动迁移、§20.7 导入反推 Blueprint 等 AI 驱动能力推迟到前端会话或后续 phase。

### 已知并行整合

- 设定集文档树（DocumentService + writing_documents 表）：由并行会话开发，与本 Phase 12 共用 writing-store.ts/types.ts。两者功能正交（documents 是设定集文档树，prose_documents 是章节正文块级模型），无逻辑冲突。整合点：softDeleteProject childTables 已含 writing_documents。
