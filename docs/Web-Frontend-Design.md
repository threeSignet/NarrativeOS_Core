...
| 用途 | 库 | 说明 |
|---|---|---|
| 富文本编辑器 | TipTap (ProseMirror) | 可扩展、支持自定义节点/标记（实体高亮、伏笔标记）|
| 图可视化 | Cytoscape.js / react-flow | 关系图、空间图、影响图 |
| 状态管理 | Zustand | 轻量、TypeScript 友好 |
| 路由 | React Router | 多视图导航 |
| UI 组件库 | Shadcn/ui + Tailwind CSS | 可定制、不锁定、设计语言可控 |
| 实时通信 | Socket.io / 原生 WebSocket | Agent 流式响应 |
| 拖拽布局 | react-mosaic / allotment | 可拖动面板布局 |
...
```
前端 (React) ←→ BFF (Node.js) ←→ 写作层 Service ←→ Core Engine
     ↑                    ↑
  WebSocket            REST API
  (Agent 流式)         (CRUD 操作)
```
...
| 实体列表（1000+项） | 列表渲染慢 | react-window / react-virtualized 虚拟滚动 |
...
interface EntityCardProps {
  ...
  entity: EntityListItem;
  onClick?: () => void;       // → 详情
  actions?: ReactNode;        // slot: approve/deprecate 按钮
}
