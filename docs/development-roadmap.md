...
| E3 | Agent 协作可见性强化 | 作者随时知道 AI 在做什么、需要确认什么 | ✅ 完成 |
...
### 迭代 D2 · RevisionService 接前端（修订历史查看器）✅（2026-07-22）

**做了什么**：
- 后端：RevisionService 补 `listAllRevisions` 方法 + writing-store 补 `listAllRevisionsByProject`（按项目列出全部修订，按时间倒序）
- BFF：`apps/bff/src/routes/revisions.ts`（2 端点：GET 列表 + GET 单条），server.ts 注册
- 前端：`api/revisions.ts`（RevisionRecord 类型 + TargetType 9种/Action 5种中文标签/颜色）+ `stores/revision.ts`（load + targetTypeFilter computed）
- 插件：`revision-history`（时钟回溯图标 + RevisionSideView 侧栏[类型过滤芯片] + RevisionListView 主区[垂直时间线列表]）+ manifest（order=10）
- 验证脚本：`scripts/verify-revision-d2.mjs`（Playwright 6 项验证）
...
### 迭代 E3 · Agent 协作可见性强化 ✅（2026-07-22）

**做了什么**：
- 前端 AgentPanel 重构（E3 协作可见性）：
  - **决策面板**：当 Agent 回合状态为 `needs_user_confirmation` 时，自动加载待确认决策并显示（类型标签+标题+描述+确认/拒绝按钮），通过 `listDecisions`/`resolveDecision` API 操作
  - **回合摘要**：Agent 回合完成后显示工具调用统计（X 个工具，Y 个成功，Z 个失败）
  - **工具调用指示器**（E1 继承）：assistant 消息内嵌工具调用状态点+中文名+完成/失败/执行中
  - **决策角标**：标题栏徽章显示待确认决策数量
  - **自动加载**：当消息状态变为 needs_user_confirmation 时自动拉取决策列表
- 验证脚本：`scripts/verify-collab-e3.mjs`（Playwright 7 项验证，含真实 Agent 调用触发决策）

**验证**：vue-tsc 0 错；后端 vitest 929/929 全绿；Playwright 7/7（面板/工具调用/回合摘要/消息/决策面板/BFF/无异常）

**★ 阶段 E 完结**：工具可见性(E1) + 正文写入(E2) + 协作可见性(E3) 全部完成。

**下一步**：规划阶段 F（产品打磨/UI 优化/性能/部署）
