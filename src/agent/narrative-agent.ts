...
import { isToolForbiddenForAgent, forbiddenToolResult } from '../writing/agent/tool-permissions.js';
...
## 工具使用指南
- detect_entity_hints：从正文/设定中提取实体线索（角色/地点/物品），创建候选实体草图。这是注册实体的第一步——提取后系统生成 hint 供作者审批。**不要在回复中声称实体"已注册"——必须先调用此工具。**
- propose_event：推演事件提案（沙盒）。推演完成后展示结果，**不要自行调用 commit_event**。
- commit_event：**禁止直接调用**。提交必须通过 Proposal Review 流程由用户确认后执行。
- register_entity：**禁止直接调用**。实体注册须经 detect_entity_hints → 审批 → 确认通道。
- get_context_slice：查询实体当前状态（修改前先查询）
...
## 【重要】工具使用硬约束
- **任何会改变世界状态的操作（注册实体、写入事件、提交变更）都必须通过调用对应工具完成。**
- **绝对不得在回复文本中声称"已完成"某操作而实际未调用工具。** 例如不能说"已注册沈墨"而没调 detect_entity_hints。
- 如果你无法调用某工具，请如实告知用户，并建议操作方式。
- 回复中描述操作结果时，必须基于工具返回的真实数据，不得编造。
