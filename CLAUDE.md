...
5. `SQLiteFactStoreAdapter.assert` 的签名是 `Omit<Fact, 'id' | 'embeddingText'>`——**不接收** `embeddingText`，由 assert 内部按 §3.1.2 格式生成（查 `entities.name` 解析主体/值的显示名）。切勿在调用处传 `embeddingText`（会被忽略）。完整谓词中文名/描述增强属 FactEmbedder + WorldPackage 职责，待后续 Phase 接入。`embedding_text` 列必须非空，否则 sync_queue consumer 兜底拼接出裸 ID 文本，污染语义检索
6. **Fact ID 必须用完整事件标识作前缀**（P1-1b 修复，2026-06-14）：`assert()` 生成 `fct_{causeEvent 去 evt_ 前缀}_{事件内序号}`。**切勿**按 `split('_')` 取前几段拼接——多词事件类型（如 `character_intro`）会让 `evt_character_intro_1_01` 误解析成 `['character','intro','1','01']` 并丢弃末尾的事件全局序号，导致同族事件（`evt_character_intro_1_01` 与 `_1_02`）生成相同 Fact ID，命中 `facts.id` 的 UNIQUE 约束，第二次 `commit_event` 抛 `UNIQUE constraint failed: facts.id` 直接失败。这是 writing-loop 场景 B/E 的真实根因（非 LLM/超时）。回归测试见 `tests/integration/fact-id-collision.test.ts`。
7. **`agent_authorized_for_session` 模式下裸 Agent 路径需自动提交**（2026-06-14）：该模式（"会话内 Agent 可自动提交，主要用于 live 验证/自动化测试"）在无 writingLayer 的裸 Agent 路径（e2e 测试用）下，`processUserInput` 必须在 `runReActLoop` 后自动调用 `handleConfirmCommit` 提交本回合 pending 提案——否则提案无限堆积在 `pendingProposalIds`，单回合 e2e（场景 A）断言 `facts>0` 必失败。注意：W1 门控拦截的是 LLM 在 ReAct 循环里**直接**调 `commit_event`（`tool-permissions.ts`），而 `handleConfirmCommit` 经授权直接调 `toolRouter.execute('commit_event')` 绕过该门控（§8.0"换调用者"）。带 writingLayer 的 CLI 路径仍走 Proposal Review（CoreBridge），其授权自动提交依赖 W13（pendingProposalIds↔ProposalView 映射），暂未在自动提交路径处理。
